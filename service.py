...
def get_book_info(book_id_v: Optional[int]=None, book_name_v: Optional[str]=None, category_name_v: Optional[str]=None, tag_name_v: Optional[str]=None):
    """
    """
    
    books = select(Book.id, Book.name, Category, Author)\
...
def update_book_info(book_id_v: Optional[int]=None, tag_id_v: Optional[id]=None):
    ...
    books = select(Book.id, Book.name, Category, Author, Tag)\
        .join(Category, Category.id==Book.category_id).\
        join(Author, Author.id==Book.author_id).\
...
def cerate_new_book(instance: BookItem):
    ...
    with session:
        ...
        new_book = Book(name=book_name, category_id=instance.category_id, author_id=author_id_v)
...
def get_user_books(user_id_v: Optional[int]=None):
    ...
    user_books = select(UserBook.id, User, Book).\
